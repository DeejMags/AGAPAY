...
export default function UserSettings() {
  ...
  const updateLocalUser = useCallback((data) => {
    try {
      ...
      console.warn('Unable to sync user locally', storageErr);
    }
  }, []);
  ...
  async function fetchUserFromBackend(id) {
    ...
    try {
      ...
      if (!res.ok || !data) {
        if (data?.error) console.warn('Unable to fetch user from backend:', data.error);
        return;
      }
      setProfile((prev) => ({
        ...
        profilePic: data.profilePic || data.photoURL || prev.profilePic,
        location: data.location || prev.location || '',
      }));
      setOriginalProfile((prev) => ({ ...(prev || {}), ...data, id: data.id || id }));
    } catch (e) {
    ...
    <div className="max-w-2xl mx-auto py-8 px-4">
      ...
      <div className="bg-white rounded-xl shadow p-6 mb-8">
        ...
        <form className="flex flex-col gap-4">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    ...
                    reader.onload = (ev) => {
                      setProfile((p) => ({ ...p, profilePic: ev.target?.result || null }));
                    };
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (currentUser && (currentUser.id || currentUser.authId || currentUser.uid)) {
                        const targetId = currentUser.id || currentUser.authId || currentUser.uid;
                        ...
                        if (!res.ok) {
                          const errorMessage = saved?.error || saved?.message || `Failed to save profile (${res.status})`;
                          setSaveStatus(errorMessage);
                          setTimeout(() => setSaveStatus(''), 3000);
                          return;
                        }
                        const normalizedId = saved?.id || targetId;
                        const updatedUser = {
                          ...currentUser,
                          ...profile,
                          ...(saved || {}),
                        ...
                        const idx = users.findIndex(
                          (u) =>
                            (u.id && normalizedId && String(u.id) === String(normalizedId)) ||
                            (u.authId && saved?.authId && String(u.authId) === String(saved.authId))
                        );
                        if (idx >= 0) {
                          users[idx] = { ...users[idx], ...saved, ...profile, id: normalizedId };
                          localStorage.setItem('agapay_users', JSON.stringify(users));
                        } else if (normalizedId) {
                          users.push({ ...(saved || {}), ...profile, id: normalizedId });
                          localStorage.setItem('agapay_users', JSON.stringify(users));
                        }
                        ...
                        setProfile({
                          ...
                          profilePic: updatedUser.profilePic || updatedUser.photoURL || null,
                      ...
                      const fallbackUser = { ...currentUser, ...profile };
                      if (!fallbackUser.id && (currentUser?.id || currentUser?.authId || currentUser?.uid)) {
                        fallbackUser.id = currentUser.id || currentUser.authId || currentUser.uid;
                      }
                      updateLocalUser(fallbackUser);
                      try {
                        const users = JSON.parse(localStorage.getItem('agapay_users') || '[]');
                        const idx = users.findIndex(
                          (u) =>
                            (u.id && fallbackUser.id && String(u.id) === String(fallbackUser.id)) ||
                            (u.authId && fallbackUser.authId && String(u.authId) === String(fallbackUser.authId))
                        );
                        if (idx >= 0) {
                      ...
                      setOriginalProfile(fallbackUser);
                      setIsEditing(false);
                      setSavedOnce(true);
                      setSaveStatus('Profile saved locally (no user id).');
                      setTimeout(() => setSaveStatus(''), 2000);
                    } catch (e) {
                      console.warn('Save failed, persisted locally', e);
                      const message = e?.message || 'Error saving profile!';
                      ...
                      try {
                        ...
                        setIsEditing(false);
                        setSavedOnce(true);
                        setSaveStatus('Saved locally (network issue)');
                        setTimeout(() => setSaveStatus(''), 2000);
                      } catch (e2) {
          ...
          {saveStatus && (
            <div className={`mt-2 text-sm ${saveStatus.includes('successfully') ? 'text-green-600' : 'text-red-600'}`}>
              {saveStatus}
            </div>
          )}
        </form>
            ...
            onClick={async () => {
              ...
              try {
                ...
                try {
                  ...
                  if (currentUser && (currentUser.id || currentUser.authId || currentUser.uid)) {
                    const targetId = currentUser.id || currentUser.authId || currentUser.uid;
                    await authFetch(`/api/users/${targetId}`, {
                      method: 'PUT',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ email: u.email, profilePic: u.photoURL }),
                    });
                  }
                } catch (persistErr) {
                  console.warn('Failed to sync Google link to backend', persistErr?.message || persistErr);
                }
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <p className="text-sm text-gray-700 mb-4">
                Your account will be deactivated and you will be signed out on all devices. You won’t be able to sign in until an admin reactivates your account. Proceed?
              </p>
              <div className="flex justify-end gap-3">
                ...
                <button
                  ...
                  onClick={async () => {
                    setDeleting(true);
                    try {
                      const res = await authFetch('/api/users/deactivate', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                      });
                      ...
                      try {
                        ...
                        localStorage.removeItem('agapay_users');
                      } catch (storageErr) {
                        /* ignore */
                      }
